...
% deadlines on status:
maxDurationUE(status(M)=proposed, status(M)=null, 10) :- motion(M).
maxDurationUE(status(M)=voting, status(M)=voted, 10) :- motion(M).
maxDurationUE(status(M)=voted, status(M)=null, 10) :- motion(M).
